...
// BuyProductHandler menangani logika pembelian produk.
func BuyProductHandler(w http.ResponseWriter, r *http.Request) {
	...
	var price int64
	var stock int
	err = tx.QueryRow("SELECT price, stock FROM products WHERE id = $1 FOR UPDATE", req.ProductID).Scan(&price, &stock)
	if err != nil {
	...
	_, err = tx.Exec("UPDATE products SET stock = $1 WHERE id = $2", newStock, req.ProductID)
	if err != nil {
	...
	_, err = tx.Exec(
		"INSERT INTO transactions (buyer_id, product_id, quantity, total_price) VALUES ($1, $2, $3, $4)",
	...
	json.NewEncoder(w).Encode(map[string]string{"message": "Pembelian berhasil"})
}
